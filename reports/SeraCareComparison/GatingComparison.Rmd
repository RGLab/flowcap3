...
```{r bcell_mse}
mse<-cbind(BCELL,fixr=fixef(mer)+resid(mer),fix=getME(mer,'X')%*%fixef(mer))
setnames(mse,"fix.V1","fix")

mse<-melt(mse[,list(Manual=crossprod(.SD[Method%in%"Manual",fix]-.SD[Method%in%"Manual",fixr])[1],OpenCyto=crossprod(.SD[Method%in%"Manual",fix]-.SD[Method%in%"OpenCyto",fixr])[1],flowDensity=crossprod(.SD[Method%in%"Manual",fix]-.SD[Method%in%"flowDensity",fixr])[1]),list(Population)],id=c("Population"))
...
```{r tcell_mse}
mse<-cbind(TCELLS,fixr=fixef(mer)+resid(mer),fix=getME(mer,'X')%*%fixef(mer))
setnames(mse,"fix.V1","fix")
...
```{r thelper_mse}
mse<-cbind(THELPER,fixr=fixef(mer)+resid(mer),fix=getME(mer,'X')%*%fixef(mer))
setnames(mse,"fix.V1","fix")
...
```{r dcmono_mse}
mse<-cbind(DC_MONO,fixr=fixef(mer)+resid(mer),fix=getME(mer,'X')%*%fixef(mer))
setnames(mse,"fix.V1","fix")
...
```{r treg_mse}
mse<-cbind(TREG,fixr=fixef(mer)+resid(mer),fix=getME(mer,'X')%*%fixef(mer))
setnames(mse,"fix.V1","fix")

mse<-melt(mse[,list(Manual=crossprod(.SD[Method%in%"Manual",fix]-.SD[Method%in%"Manual",fixr])[1],OpenCyto=crossprod(.SD[Method%in%"Manual",fix]-.SD[Method%in%"OpenCyto",fixr])[1]),list(Population)],id=c("Population"))
