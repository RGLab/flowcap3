...
gating(gating_template, gs_pregate, mc.cores = 10, parallel_type = "multicore",
       prior_group = "Center")
...
add(gs_bcell, boundary_gates)
recompute(gs_bcell)

nonDebris_gates <- getGate(gs_pregate, "nonDebris")
add(gs_bcell, nonDebris_gates, parent = "boundary")
recompute(gs_bcell)

lymph_gates <- getGate(gs_pregate, "lymph")
add(gs_bcell, lymph_gates, parent = "nonDebris")
recompute(gs_bcell)
